...
data_2d = np.load("../../../0922/data/data_2d.npy")
log_scaler = LogScaler()
log_scaler.scaling(data_2d) #ログデータ生成（self.log_data)
input_2d_data = log_scaler.log_data[:, :3, :, :]
input_temp_data = np.load("../../../0922/data/temperature.npy")
input_time_data = np.load("../../../0922/data/time.npy")
...
def test(model, test_loader, criterion, y_test_tensor, x_test_tensor):
    ...
    #r2スコア算出
    score = r2score(torch.Tensor(scaler.inverse_transform(model(x_test_tensor.to(gpu)).to(cpu).detach().numpy())).flatten(), y_test_tensor.flatten())
