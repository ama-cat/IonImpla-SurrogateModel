...
class Normalize2D():
    ...
    def __init__(self, train):
        self.train = train
        self.mean = train.permute([1, 0, 2, 3]).reshape(self.train.shape[1], -1).mean(axis=1) #チャンネルごとの平均
        self.std = train.permute([1, 0, 2, 3]).reshape(self.train.shape[1], -1).std(axis=1) #チャンネルごとの標準偏差
...
class Standardize2D():
    ...
    def __init__(self, train):
        self.train = train
        self.max = train.permute([1, 0, 2, 3]).reshape(self.train.shape[1], -1).max(axis=1)[0] #チャンネルごとの最大値
